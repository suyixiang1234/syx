...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid">
					...
					<c:when test="${not empty userList}">
						...
						<c:forEach items="${userList}" var="user" varStatus="vs">
							...
							<tr>
								...
								<td>${user.IP}</td>
								<td style="width: 90px;">
									<a class='btn btn-mini btn-info' title="绑定标签" onclick="userBindTag('${user.USER_ID }','${user.USERNAME }');"><i class='icon-edit'></i></a>
									<a class='btn btn-mini btn-info' title="绑定锚点" onclick="userBindAnchor('${user.USER_ID }','${user.USERNAME }');"><i class='icon-edit'></i></a>
								</td>
								<td style="width: 60px;">
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		//绑定标签弹窗
		function userBindTag(Id, name){
			 top.jzts();
			 var diag = new top.Dialog();
			 diag.Drag=true;
			 diag.Title = "用户（"+name+"）绑定标签";
			 diag.URL = '<%=basePath%>tag/list.do?user_id='+Id;
			 diag.Width = 750;
			 diag.Height = 640;
		...
		//绑定标签弹窗
		function userBindAnchor(Id, name){
			 top.jzts();
			 var diag = new top.Dialog();
			 diag.Drag=true;
			 diag.Title = "用户（"+name+"）绑定锚点";
			 diag.URL = '<%=basePath%>anchor/list.do?user_id='+Id;
			 diag.Width = 750;
			 diag.Height = 640;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
