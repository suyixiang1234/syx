...
public class HeadController extends BaseController {
	...
	public ModelAndView goEditEmail() throws Exception{
		...
		pd.put("HOST", Tools.readTxtFile(Const.HOST));	
		pd.put("TOPIC", Tools.readTxtFile(Const.TOPIC));		
		pd.put("userName", Tools.readTxtFile(Const.userName));		
		pd.put("passWord", Tools.readTxtFile(Const.passWord));	
		pd.put("BAOJING", Tools.readTxtFile(Const.BAOJING));
	...
	public ModelAndView saveSys() throws Exception{
		...
		/*Tools.writeFile(Const.HOST,pd.getString("HOST"));	
		Tools.writeFile(Const.TOPIC,pd.getString("TOPIC"));	
		Tools.writeFile(Const.userName,pd.getString("userName"));	
		Tools.writeFile(Const.passWord,pd.getString("passWord"));	
		Tools.writeFile(Const.BAOJING,pd.getString("BAOJING"));	*/
	
		Tools.writeFile(Const.userName,pd.getString("userName"));	
		Tools.writeFile(Const.passWord,pd.getString("passWord"));
		Tools.writeFile(Const.EMAIL,pd.getString("EMAIL"));	
		Tools.writeFile(Const.SMS1,pd.getString("SMS1"));
		
		
		Tools.writeFile(Const.HOST,pd.getString("HOST"));	
		Tools.writeFile(Const.TOPIC,pd.getString("TOPIC"));	//交易手续
		Tools.writeFile(Const.BAOJING,pd.getString("BAOJING"));//提现手续费
...
*/		mv.addObject("msg","OK00");
	...
	public void zntask() throws Exception{
		PageData pd = new PageData();
		coinsService.edit1(pd);
