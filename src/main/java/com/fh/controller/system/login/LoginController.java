...
import websocket.socket.WebSocketHander;
/*
 * 总入口
 */
@Controller
public class LoginController extends BaseController {
			...
			if(false){
				...
				if(true){
					...
					if(pd != null){
						...
						user.setDepartment_id(pd.getString("ZD_ID"));
	 ...
	 * 访问系统首页
	 */
	@RequestMapping(value="/adminInfo/{changeMenu}")
		...
		try{
			...
			if (user != null) {
				...
				//FusionCharts 报表
			 	...
			 	//FusionCharts 报表
				if("indexhj".equals(changeMenu)){
			 		mv.setViewName("system/HJ/index");
			 	}else if("indexwin7".equals(changeMenu)){
			 		mv.setViewName("system/win7/index");
			 	}else if("indexlte".equals(changeMenu)){
			 		mv.setViewName("system/LTE/index");
			 	}else if("indexace".equals(changeMenu)){
	...
	public ModelAndView logout(){
		...
		if (WebSocketHander.userSession.size()!=0) {
			WebSocketHander.userSession.remove(user.getUSERNAME());
		}
