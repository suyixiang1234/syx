...
<body>
	<form action="happuser/editU2.do" name="userForm" id="userForm" method="post">
		...
		<table>
			...
			<tr>
				<td>
					<select name="is_rz" title="实名审核">
					<option value="0" <c:if test="${pd.is_rz == 0 }">selected</c:if> >未认证</option>
					<option value="1" <c:if test="${pd.is_rz == 1 }">selected</c:if> >审核中</option>
					<option value="2" <c:if test="${pd.is_rz == 2 }">selected</c:if> >已认证</option>
					</select>
				</td>
				<td>
		    ...
		    <tr>
				...
				<td>
					<input type="text" name="moneys" id="moneys"  value="${pd.moneys }" placeholder="" />
				</td>
				<td>
					<input type="text" name="PASSWORD" id="PASSWORD"  placeholder="需要修改密码，请输入！" />
				</td>
			</tr>
